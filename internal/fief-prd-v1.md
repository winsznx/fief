...
8. Rental terms (correction 2026-08-20, surfaced by the frontend handoff Q3): RentalDesk.list now carries termSeconds so a renter sees the grant's expiry before confirming; rent derives expiry = block.timestamp + termSeconds and maxDecisions = msg.value / feePerDecisionWei. Also reaffirmed (it was ambiguous in the frontend handoff, now fixed there): the on-chain entries[] array is accepted-only; a rejected decision reverts or emits DecisionRejected via recordDecisionStrict, so it is never stored and has no entry index — the record ledger is accepted-only and the green/red pair is a proof artifact. (Affects section 5.)
...
### RentalDesk
- list(tokenId, feePerDecisionWei, minEscrow, termSeconds): owner-only. termSeconds is the rental duration so a renter sees the grant's expiry before confirming (correction 2026-08-20, surfaced by frontend Q3).
- rent(tokenId) payable: expiry = block.timestamp + termSeconds; maxDecisions = msg.value / feePerDecisionWei; creates grant { renter, expiry, maxDecisions, remainingEscrow } keyed by (tokenId, renter) in RentalDesk state, then calls FiefAgent.authorizeUsage(tokenId, renter) (draft 2-arg form; grantId is NOT passed on-chain, correction 2026-08-19).
