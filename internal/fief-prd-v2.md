...
| 3 | **Agentic ID is not usable on mainnet.** Revalidated 2026-08-25 by `eth_getCode`: the AgenticID and ReputationRegistry proxies have zero bytecode on 16661; the canonical ERC-8004 address has 130 bytes of ERC-1967 proxy whose every read reverts, while the same address on testnet answers `name()` with `AgentIdentity`. Original note below.<br><br>**Agentic ID has no mainnet deployment.** `constants.d.ts` on `ZERO_G_MAINNET`: "NOTE: AgenticID is testnet-only today - no mainnet contract deployment exists yet; this is here for when the protocol goes to mainnet." Contract addresses are not baked into the SDK at all; they are a per-deployment artifact | SRC (constants.d.ts) | **Corrects patch.md.** ERC-7857/8004 cannot carry the Wave 3 mainnet requirement. Fief's own contracts go to mainnet; Agentic ID + ERC-8004 run on testnet 16602 and are labelled testnet everywhere |
