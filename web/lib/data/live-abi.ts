/**
 * Contract ABIs for the browser, copied verbatim from `runtime/src/abi.ts`.
 *
 * Copied rather than imported because `web` must not depend on the runtime's
 * node-only dependency tree. The Foundry suite is what guarantees these
 * signatures are right, and a drift surfaces immediately as a decode failure on
 * the first read.
 */
